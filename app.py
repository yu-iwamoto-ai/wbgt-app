...
with tab1:
    st.header("新規データの登録")
    sub_tab_a, sub_tab_sky, sub_tab_b = st.tabs(["🌡️ WBGT・気象数値登録", "🌤️ 空の写真登録", "🧱 表面温度・写真登録"])
    
    # --------------------------------------
    # サブタブA: WBGT数値 ＋ 風の状況
    # --------------------------------------
    with sub_tab_a:
        st.subheader("1. WBGT・風の状況等の入力")
        ...
        with c_time_a:
            ...
            time_a_str = st.text_input("観測時刻 (手入力)", value=now_time_str, key="time_a", help="例: 15:30")
        ...
        wind_a = st.selectbox("風の有無を選択", WINDS, key="wind_a")
        
        uploaded_main_a = st.file_uploader("📷 測定機器の証拠写真（任意）", type=["jpg", "jpeg", "png"], key="img_a")
        
        if st.button("数値・風データを登録する", type="primary", key="btn_a"):
            try:
            ...
            if not df[match_mask].empty:
                ...
                db_container["df"].loc[idx, "風"] = wind_a
                ...
                new_row = pd.DataFrame([[dt_str_a, loc_a, "-", wind_a, wbgt_a, ta_a, rh_a, "-", judgment, img_name, "-", "-"]], 
    ...
    # --------------------------------------
    with sub_tab_sky:
        st.subheader("2. 校内全体の「空の写真」登録")
        st.info("※地点の選択は不要です。撮影日付・時刻を選択・入力して保存してください。")
        
        c_date_sky, c_time_sky = st.columns(2)
        with c_date_sky:
            date_sky = st.date_input("撮影日を選択", date.today(), key="date_sky")
        with c_time_sky:
            now_time_str = datetime.now().strftime("%H:%M")
            time_sky_str = st.text_input("撮影時刻 (手入力)", value=now_time_str, key="time_sky", help="例: 15:30")
            
        time_part_sky = time_sky_str.strip()
        if len(time_part_sky) == 5: time_part_sky += ":00"
        elif len(time_part_sky) == 4 and ":" in time_part_sky: time_part_sky = "0" + time_part_sky + ":00"
        dt_str_sky = f"{date_sky} {time_part_sky}"
        
        weather_sky = st.selectbox("空の状況（天候）", WEATHERS, key="weather_sky")
        uploaded_sky = st.file_uploader("🌤️ 空の写真を選択", type=["jpg", "jpeg", "png"], key="sky_only")
        
        if st.button("空の写真を保存する", type="primary", key="btn_sky"):
            try:
                datetime.strptime(dt_str_sky, "%Y-%m-%d %H:%M:%S")
            except ValueError:
                st.error("時刻の入力形式が正しくありません。「15:30」のように入力してください。")
                st.stop()
                
            dt_stamp = f"{date_sky.strftime('%Y%m%d')}_{time_part_sky.replace(':', '')}"
            sky_img_name = save_uploaded_image(uploaded_sky, "sky", dt_stamp, "全体")
            
            df = db_container["df"]
            match_mask = (df["日時"] == dt_str_sky) & (df["地点"] == "校内全体（全地点共通）")
            
            if not df[match_mask].empty:
                idx = df[match_mask].index[-1]
                db_container["df"].loc[idx, "天候"] = weather_sky
                if sky_img_name != "-": db_container["df"].loc[idx, "空画像"] = sky_img_name
                st.success(f"【更新完了】 空の写真を更新しました ({dt_str_sky})")
            else:
                new_row = pd.DataFrame([[dt_str_sky, "校内全体（全地点共通）", weather_sky, "-", 0.0, 0.0, 0.0, "-", "データなし", "-", sky_img_name, "-"]], 
                                       columns=REQUIRED_COLUMNS)
                db_container["df"] = pd.concat([db_container["df"], new_row], ignore_index=True)
                st.success(f"【登録完了】 空の写真を保存しました ({dt_str_sky})")
    ...
    # --------------------------------------
    with sub_tab_b:
        st.subheader("3. 地面の表面温度データの入力")
        loc_b = st.selectbox("対象地点を選択", LOCATIONS_ENV, key="loc_b")
        ...
        with c_time_b:
            now_time_str = datetime.now().strftime("%H:%M")
            time_b_str = st.text_input("観測時刻 (手入力)", value=now_time_str, key="time_b", help="例: 15:30")
        ...
        if st.button("表面温度データを登録する", type="primary", key="btn_b"):
            try:
            ...
            if not df[match_mask].empty:
                ...
                st.success(f"【更新完了】 {loc_b} ({dt_str_b}) の表面温度を更新しました！")
            else:
                new_row = pd.DataFrame([[dt_str_b, loc_b, "-", "-", 0.0, 0.0, 0.0, surface_temp_b, "データなし", "-", "-", surf_img_name]], 
                                       columns=REQUIRED_COLUMNS)
                db_container["df"] = pd.concat([db_container["df"], new_row], ignore_index=True)
                st.success(f"【登録完了】 {loc_b} ({dt_str_b}) の表面温度データを保存しました！")
...
with tab2:
    ...
    view_tab_a, view_tab_b = st.tabs(["🌡️ WBGT最新状況", "🧱 表面温度最新状況"])
    df = db_container["df"]

    with view_tab_a:
        # 空写真の全校共通最新データを取得
        sky_df = df[(df["空画像"].notna()) & (df["空画像"] != "-")]
        common_sky_img = None
        common_sky_dt = "-"
        common_weather_str = "-"
        
        if not sky_df.empty:
            latest_sky_row = sky_df.iloc[-1]
            raw_sky_dt = str(latest_sky_row.get("日時", ""))
            common_sky_dt = raw_sky_dt[5:16].replace('-', '/') if len(raw_sky_dt) >= 16 else raw_sky_dt
            common_weather_str = latest_sky_row.get("天候", "-")
            sky_f = latest_sky_row.get("空画像", "-")
        ...
        if common_sky_img:
            st.subheader("🌤️ 最新の空模様")
            st.image(common_sky_img, caption=f"撮影時刻: {common_sky_dt} （天候: {common_weather_str}）", use_container_width=True)
            st.write("---")
        ...
        for idx, loc in enumerate(LOCATIONS_WBGT):
            ...
            with st.container():
                if not wbgt_records.empty:
                    latest_wbgt_row = wbgt_records.iloc[-1]
                    
                    wbgt_val = latest_wbgt_row.get("WBGT", 0)
                    judgment_text = latest_wbgt_row.get("判定", "データなし")
                    ta_val = latest_wbgt_row.get("気温", 0)
                    rh_val = latest_wbgt_row.get("湿度", 0)
                    wind_val = latest_wbgt_row.get("風", "-")
                    img_file = latest_wbgt_row.get("画像", "-")
                    wbgt_dt_str = str(latest_wbgt_row.get("日時", ""))
                    ...
                    st.markdown(
                        ...
                        <div style="border-left: 6px solid {color}; padding: 10px 14px; background-color: #f8f9fa; border-radius: 6px; box-shadow: 1px 1px 3px rgba(0,0,0,0.05); margin-bottom: 6px;">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 12px 16px; margin-top: 8px; font-size: 0.9rem; color: #333;">
                                ...
                                <span style="margin-left: auto; color: #888; font-weight: bold; font-size: 0.8rem;">📅 測定: {disp_dt}</span>
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True
    ...
    with view_tab_b:
        st.subheader("校内地面表面温度の最新情報")
        for idx, loc in enumerate(LOCATIONS_ENV):
            mapped_target = LOCATION_MAPPING.get(loc, None)
            loc_df = df[(df["地点"] == loc) | (df["地点"] == mapped_target)] if mapped_target else df[df["地点"] == loc]
            
            surf_records = loc_df[loc_df["表面温度"] != "-"]
            
            with st.container():
                if not surf_records.empty:
                    latest_row = surf_records.iloc[-1]
                    ...
                    st.markdown(
                        ...
                        <div style="border-left: 6px solid #2ECC71; padding: 8px 12px; margin-bottom: 4px; background-color: #f8f9fa; border-radius: 4px; box-shadow: 1px 1px 2px rgba(0,0,0,0.05);">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <span style="color: #888; font-weight: bold; font-size: 0.8rem;">📅 測定: {formatted_dt_str}</span>
                            </div>
                            <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-top: 6px; font-size: 0.85rem; color: #444;">
                                <span><b>地面表面温度:</b> <span style="color:#D35400; font-weight:bold; font-size:1rem;">{surf_disp}</span></span>
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True
